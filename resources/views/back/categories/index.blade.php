                        ...
                        <div class="col-xl-12">
                            <div class="card">
                                <div class="card-header align-items-center d-flex">
                                    ...
                                    <a href="{{route('categories.create')}}" class="btn btn-success btn-label">
                                            ...
                                            <table  id="example" class="table table-bordered dt-responsive nowrap table-striped align-middle" style="width:100%">
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr  data-id="{{ $row->id }}">
                                                        ...
                                                        <td>{{@$row->categoryTranslations->where('locale','en')->first()->name}}</td>

                                                        <td>
                                                            @if($row->children->count())
                                                                <a href="{{route('categories.childCategory',['category' => $row->id])}}" class="btn btn-success">Child Categories</a>
                                                            @endif
                                                        </td>
                                                        
                                                        <td>
                                                           
                                                                <a href="{{route('categories.optionGroups',['category' => $row->id])}}" class="btn btn-success">Option Groups</a>
                                                            
                                                        </td>
                                                        
                                                        <td>
                                                            <div class="hstack gap-3 flex-wrap">
                                                                <a href="{{ route('categories.edit', ['category' => $row->id]) }}" class="link-success fs-15"><i class="ri-edit-2-line"></i></a>
        ...
        $("#example tbody").sortable({
            update: function (event, ui) {
                // Get the new row order
                    ...
                    $.ajax({
                        url: "{{ route('datatable.updateOrder') }}",
                        type: "GET",
                        data: {
                            order: newOrder,
                        }
